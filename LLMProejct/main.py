...
class FluxDataLoader(Dataset):
    def __init__(self, root_path, flag=None, on_enhance=False, encoder="bert", on_mm_statistics=False, on_mm_history=False):
        self.flag = flag
        self.encoder = encoder
        self.on_mm_statistics = on_mm_statistics # 多模态-统计信息
        self.on_mm_history = on_mm_history # 多模态-历史记录
...
class CustomDataModule(LightningDataModule):
    def __init__(self, root_path, batch_size=16, num_workers=10, encoder=None, on_mm_statistics=False, on_mm_history=False, on_enhance=False):
        super().__init__()
...
class MyTransformerModel(nn.Module):
    def __init__(self, num_classes=2, input_dim=1, model_type="bert", use_lora=False, text_emb_dim=768, use_multimodal=False): #  input_dim
        ...
        # 加载预训练模型
        if self.model_type == "bert" or self.model_type == "roberta-c":
...
if __name__ == "__main__":
    ...
    # 定义调用的模型: bert、gpt2、roberta、moment
    ...
    # 是否一键开启所有创新点
    parser.add_argument('--all', action='store_true', help='Enable physical loss')

    parser.add_argument("--exp_id", type=str, default=None, # 指定实验id，避免日志写入权限冲突
                        help="Experiment ID (used as log directory name). "
                             "If not provided, auto-generate timestamp.")
    ...
    if args.all:
        ...
        args.on_mm_statistics = True
        args.on_mm_history = True
