...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        x = x + self.dropout(new_x) # after. x'shape (32, 96, 512)
        x, _ = self.decomp1(x) # after. x'shape (32, 96, 512)
        y = x
        y = self.dropout(self.activation(self.conv1(y.transpose(-1, 1))))
        y = self.dropout(self.conv2(y).transpose(-1, 1)) # after. y'shape (32, 96, 512)
    ...
    def forward(self, x, attn_mask=None):
        ...
        if self.conv_layers is not None:
            ...
            for attn_layer in self.attn_layers:
                x, attn = attn_layer(x, attn_mask=attn_mask) # 第一次attn_layer, x. (32, 96, 512); 第二次, x. (32, 96, 512) attn都是none
                attns.append(attn)
